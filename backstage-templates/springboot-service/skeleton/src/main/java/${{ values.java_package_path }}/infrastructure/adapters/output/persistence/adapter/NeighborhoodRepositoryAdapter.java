...
import ${{ values.java_package_name }}.domain.ports.output.NeighborhoodRepositoryPort;
...
public class NeighborhoodRepositoryAdapter implements NeighborhoodRepositoryPort {
    ...
    @Override
    public List<Neighborhood> findByFilters(String search, String status, String dateFrom, String dateTo, Integer page, Integer size) {
        log.debug("Searching Neighborhoods with filters - search: {}, status: {}, dateFrom: {}, dateTo: {}, page: {}, size: {}", 
        ...
        try {
            ...
            log.error("Database error while filtering Neighborhoods: {}", e.getMessage(), e);
            throw new InternalServerErrorException("Failed to filter Neighborhoods", e);
        }
    }
    ...
    @Override
    public List<Neighborhood> findAllPaged(Integer page, Integer size) {
    ...
    public Page<Neighborhood> findAllPagedInternal(org.springframework.data.domain.Pageable pageable) {
