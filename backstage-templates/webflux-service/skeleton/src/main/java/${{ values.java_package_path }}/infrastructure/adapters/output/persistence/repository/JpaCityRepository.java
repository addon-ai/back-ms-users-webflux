...
public interface JpaCityRepository extends R2dbcRepository<CityDbo, UUID> {
     ...
     */
    @Query("SELECT * FROM cities e WHERE " +
     ...
     */
    @Query("SELECT COUNT(*) FROM cities e WHERE " +
           "(:search IS NULL OR :search = '' OR " +
           "LOWER(e.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.status) LIKE LOWER(CONCAT('%', :search, '%')))")
    ...
    @Query("SELECT * FROM cities e WHERE " +
           ...
           "ORDER BY e.created_at DESC " +
           "LIMIT :limit OFFSET :offset")
    Flux<CityDbo> findByFilters(@Param("search") String search,
     ...
     */
    @Query("SELECT * FROM cities e ORDER BY e.created_at DESC LIMIT :limit OFFSET :offset")
