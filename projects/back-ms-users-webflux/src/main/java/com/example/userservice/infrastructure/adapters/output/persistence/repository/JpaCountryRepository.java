...
public interface JpaCountryRepository extends R2dbcRepository<CountryDbo, UUID> {
     ...
     */
    @Query("SELECT * FROM countries e WHERE " +
     ...
     */
    @Query("SELECT COUNT(*) FROM countries e WHERE " +
           "(:search IS NULL OR :search = '' OR " +
           "LOWER(e.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.status) LIKE LOWER(CONCAT('%', :search, '%')))")
    ...
    @Query("SELECT * FROM countries e WHERE " +
           ...
           "ORDER BY e.created_at DESC " +
           "LIMIT :limit OFFSET :offset")
    Flux<CountryDbo> findByFilters(@Param("search") String search,
     ...
     */
    @Query("SELECT * FROM countries e ORDER BY e.created_at DESC LIMIT :limit OFFSET :offset")
